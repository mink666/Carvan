    ...
    <form action="{{ route('Admin.RangesMgr.update',['id' => $range->id]) }}" method="POST" enctype="multipart/form-data" class="flex w-full">
        ...
        <div class="w-1/2 p-6 flex flex-col">
            ...
            <div class="space-y-4 flex-grow">
                ...
                <div>
                    <label class="block text-base font-medium text-gray-700 mb-1">Active</label>
                    <select name="is_active" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black">
                        <option value="1" {{ old('is_active', $range->is_active ?? 1) == 1 ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ old('is_active', $range->is_active ?? 1) == 0 ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
            </div>
