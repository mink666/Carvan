...
@section('title', $rangeOfCar->name . ' Cars - Discover Our Collection') {{-- Tiêu đề trang động --}}

@section('content')
<div class="bg-gray-100 min-h-screen">

    {{-- 1. Range Hero Section (Sử dụng ảnh nền) --}}
    <section class="range-hero relative text-white py-20 md:py-32 text-center bg-gray-800">
        {{-- Ảnh nền --}}
        <div class="absolute inset-0">
            {{-- GIẢ SỬ bạn có một trường 'banner_image' trong $rangeOfCar, hoặc dùng ảnh mặc định --}}
            <img src="{{ $rangeOfCar->banner_image ? asset($rangeOfCar->banner_image) : asset('images/banners/default_range_banner.jpg') }}"
                 alt="Showcasing {{ $rangeOfCar->name }} vehicles"
                 class="w-full h-full object-cover opacity-50"> {{-- Điều chỉnh opacity để text dễ đọc --}}
        </div>
        {{-- Lớp phủ màu để làm nổi text hơn (tùy chọn) --}}
        ...
        <div class="container mx-auto px-4 relative z-10">
            ...
            @endif
            {{-- Nút Call to Action nhỏ trên banner nếu muốn --}}
            {{-- <div class="mt-8">
                <a href="#car-models-in-range" class="cta-button-light">View Models</a>
            </div> --}}
        </div>
    </section>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                    @foreach($rangeOfCar->carModels as $model)
                        <div class="bg-white rounded-xl shadow-xl overflow-hidden group flex flex-col hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:-translate-y-2 hover:scale-[1.02]">
                            <a href="{{ route('car_models.show', $model->id) }}" class="block overflow-hidden relative">
                                @if($model->image)
                            ...
                            <div class="p-5 flex flex-col flex-grow">
                                ...
                                {{-- Không cần hiển thị lại tên dòng xe ở đây vì đã ở trang của dòng xe đó --}}
